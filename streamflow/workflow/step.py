...
class GatherStep(BaseStep):
    ...
    async def run(self):
        ...
        while tasks:
            ...
            for task in finished:
                ...
                if check_termination(token):
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(
                            f"Step {self.name} (wf {self.workflow.name}) received termination token on port {task_name}"
                        )
                else:
                    if task_name == "__size__":
                        ...
                        port = input_port
                        size_token = self.size_map.get(key)
                        if (
                            len(self.token_map.setdefault(key, [])) == size_token.value
                            if size_token
                            else None
                        ):
        ...
        for key in (k for k in self.token_map.keys() if k not in keys_completed):
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Step {self.name} (wf {self.workflow.name}) forces gather on key {key}"
                )
            await self._gather(key)
        # Terminate step
