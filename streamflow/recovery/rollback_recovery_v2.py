...
class ProvenanceGraph:
    ...
    def remove_prevs(self, token_id):
        empty_dependecies = set()
        for prev_token_id in self.dag_tokens.prev(token_id):
            self.dag_tokens[prev_token_id].remove(token_id)
            if (
                not self.dag_tokens[prev_token_id]
                and prev_token_id != DirectGraph.INIT_GRAPH_FLAG
            ):
                empty_dependecies.add(prev_token_id)
        for token_id in empty_dependecies:
            self.remove_prevs(token_id)
            self.port_name_ids.pop(token_id)
