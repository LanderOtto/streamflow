...
def is_in_a_list(step, list1, list2):
    for elem in list1:
        if step == elem:
            return True
    for elem, _ in list2:
...
def add_elem(port, steps_found, condition, create_elem):
    for step in port.get_input_steps():
        if condition(step):
            steps_found.append(create_elem(step))

async def _search_lost_data_steps(original_workflow, to_visit):
    visited = []
    condition = lambda x : not is_in_a_list(x, to_visit, visited)
    elem_to_visit = lambda x : x
    elem_visited = lambda x : (x, True)
    suspended = []
    while len(to_visit) > 0:
        current_step = to_visit.pop()
        visited.append((current_step, False))
        for step_port_name, port in current_step.get_input_ports().items():
            ...
            if isinstance(port, JobPort):
                add_elem(port, to_visit, condition, elem_to_visit)
            else:
                # res = any([contains_file(t) for t in port.token_list])
                files = [ inner_token for outer_token in port.token_list for inner_token in get_file(outer_token) ]
                if files:
                    ...
                    restart = True
                    for file in files:
                        restart = restart and not tmp_is_local(current_step, file['path'])
                    if restart:
                        add_elem(port, to_visit, condition, elem_to_visit)
                    else:
                        add_elem(port, visited, condition, elem_visited)
                else:
                    add_elem(port, suspended, condition, elem_to_visit)
    for step in suspended:
        if not is_in_a_list(step, [], visited):
            visited.append((step, True))
    return visited



async def _create_graph(original_workflow: Workflow, original_step: Step, context, loading_context, new_workflow: Workflow):
    ...
    for key, in_port in original_input_ports.items():
        input_ports_loaded[key] = await loading_context.load_port(context, in_port.persistent_id)
        input_ports_loaded[key].workflow = new_workflow
    for key, out_port in original_step.get_output_ports().items():
        output_ports_loaded[key] = await loading_context.load_port(context, out_port.persistent_id)
...
def search_step_by_output_port(searched_port, workflow):
    ...
    raise FailureHandlingException(f"Step not found by output port {searched_port.name}")
...
class DefaultFailureManager(FailureManager):
    ...
    async def _are_available_job_data(self, job: Job, connector: Connector, location: Location) -> bool:
        ...
        for directory in directories:
            exists_tasks.append(
                asyncio.create_task(
                    remotepath.exists(connector, location, directory)
                )
            )
                    ...
                    for token in job.inputs.items():
                        if isinstance(token, CWLFileToken):
                            inputs_path_exist.append(
                                asyncio.create_task(remotepath.exists(connector, location, token.value.path)))
                else:
        ...
        if not exists_results[0] or not all(await asyncio.gather(*inputs_path_exist)):
            logger.debug(f"Handling failure job {job.name}: invalidating all data in the {location} location")
    ...
    async def _recover_data(self, job_version: JobVersion, job: Job):
        ...
        context = step_failed.workflow.context # build_context(workflow.context.config_dir, workflow.context
        ...
        inner_output_ports = []
        rollback_steps = await _search_lost_data_steps(workflow, [step_failed])
        for curr_step in [ s for s, _ in rollback_steps]:
            output_ports = await _create_graph(workflow, curr_step, context, loading_context, new_workflow)
        ...
        workflow_input_ports = [ search_step_by_output_port(port, workflow) for port_name, port in new_workflow.ports.items() if port not in inner_output_ports ]
        ...
        for step, input_port in workflow_input_ports:
            # token = await workflow.ports[input_port.name].get(step.name)
        ...
        for key, port in new_workflow.steps[step_failed.name].get_output_ports().items():
            new_workflow.output_ports[key] = port.name
        # controllare che output_ports ci sono dentro new_workflow ...
        # probabile quelli di workflow, capire che chiave mettere al dizionario (invece di recovery_i)


        # TODO: pensare a come dare i put iniziali alle porte di ingresso del workflow
    ...
    async def _replay_job(self, job_version: JobVersion) -> CommandOutput:
        ...
        if self.max_retries is None or self.jobs[job.name].version < self.max_retries:
            ...
            try:
                ...
                available_locations = await connector.get_available_locations(service=allocation.target.service)
