...
async def _search_lost_steps(original_workflow, step_failed):
    ...
    # steps which inputs are available or they are generated by other steps inside the list
    visited = []

    # steps which at least one input is available, but in that moment of analysis the information is not enough to know if all the inputs are available.
    suspended = []
        ...
        for step_port_name, port in current_step.get_input_ports().items():
            ...
            if isinstance(port, JobPort):
                ...
                if files:
                    ...
                    restart = True
                    for file in files:
                        restart = restart and not tmp_is_local(
                            current_step, file["path"]
                        )
                    if restart:
                        add_steps_by_port(port, to_visit, condition)
                    else:
                        add_steps_by_port(port, visited, condition)
                else:
                    add_steps_by_port(port, suspended, condition)
    for step in suspended:
        if not contains(step, visited):
            visited.append(step)
    return visited
